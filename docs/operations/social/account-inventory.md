...
| DEV Community | Brand account `joinaireligion` created; owner reports email verification completed, but the browser session is not yet signed in | Article adapter is implemented behind `DEV_PUBLISHING_ENABLED`; API key is not yet stored and no article has been published | Sign in, create a narrowly named API key, store it in the production secret store, then verify one relevant English technical draft/canary |
| Tumblr | Live primary blog: `joinaireligion` | OAuth1 owner authorization and API identity were verified; a private integration draft was created successfully; credentials are held only in macOS Keychain and production publishing remains disabled | Move credentials directly to the VPS secret store, run the updated adapter against one reviewed draft, then enable only after external verification |
| Open distribution remainder (10 channels) | No verified external accounts, approvals, credentials, or live publications | Flipboard feed metadata plus Apple News, Blogger, Hashnode, Ghost, LINE, Lemmy, MediaWiki, Fandom, and Nostr adapters are implemented behind independent default-off gates | Follow [open distribution onboarding](./open-distribution-onboarding.md), complete one provider gate at a time, then externally verify a single canary |
