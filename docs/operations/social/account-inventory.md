...
| LinkedIn | Live organization Page: `joinaireligion`; organization ID `143125933`; the personal account is retained only as the non-public administrator; one public Page-authored UI post is verified | Text adapter exists behind `LINKEDIN_PUBLISHING_ENABLED` and now fails closed unless the author is exactly `urn:li:organization:143125933`; the provider remains disabled and no app approval, OAuth credential, API canary, or public API-authored result is verified | Owner submits the app and terms, then a Page super admin verifies it; confirm registered-legal-organization eligibility and submit Community Management Development access, approve only `w_organization_social` OAuth, verify one API canary, then complete Standard review before production enabling |
| Threads | Live: `@joinaireligion`; public profile, avatar, bio, site link, seven public Threads, and current 2026-08-11 article activity are verified | Live text publishing is verified and uncertain create/publish outcomes are now terminal `AMBIGUOUS`; public evidence does not prove token health, and expiry/refresh tracking is not implemented | Add token-expiry/refresh health; until then report token health as unknown and reauthorize only on an authorization failure |
...
| --- | --- | --- | --- |
| 1 | LinkedIn organization | Organization Page and public identifier, organization-only text-post adapter, locale copy, provider gating, idempotency, and delivery logs; developer-app name, Page, and privacy-policy fields are prepared | Owner submits the app/terms and Page-super-admin verification; after legal-organization eligibility and Development approval, authorize only `w_organization_social`, verify one API canary, and complete Standard review before production |
