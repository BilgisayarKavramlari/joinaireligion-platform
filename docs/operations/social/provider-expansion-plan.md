...
| LinkedIn | Organization Page, developer app, verified Page association and business email, and first Page-authored UI post live; adapter not yet enabled | Public Page `https://www.linkedin.com/company/joinaireligion/`, organization ID `143125933`, developer app ID `262900168`, verified Page association and business email, and manual Page post `https://www.linkedin.com/feed/update/urn:li:share:7492772987650134016/` were verified. The personal account remains the non-public administrator. The Community Management Development form is open at the registered legal-name question; no OAuth credential or API-delivery evidence exists yet. |
...
| Threads | Live publication verified; public profile metadata showed seven Threads and current 2026-08-11 article activity during the latest audit | Text-container publishing, status polling, permalink capture, multilingual policy, and terminal ambiguity handling are implemented; proactive token-expiry/refresh health is still missing. |
...
Verified on 2026-08-12: developer app `262900168` was created, associated with Page `143125933`, and the Page administrator completed the irreversible company-association verification and business-email confirmation. Community Management Development is paused at the form's registered active-business legal-name question; no name is inferred or fabricated. The portal still shows no saved privacy-policy URL despite repeated valid update attempts, so that field remains a provider-portal issue rather than a completed gate. Remaining gates are accurate registered-legal-organization Development review, three-legged OAuth with only `w_organization_social`, one organization API canary, and Standard-tier screencast review. Never request member-post scopes or use a person/member author.
