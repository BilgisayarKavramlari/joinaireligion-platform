...
Remaining provider gates: create a new app with no other API products, associate it with the Page, have a Page super admin verify it, and pass LinkedIn's Community Management Development review. As of the 2026-08-11 official documentation, eligibility is limited to registered legal organizations and commercial use cases, with verified business email, legal organization data, website and privacy policy required. After approval, complete three-legged OAuth with only `w_organization_social` for the canary; never request member-post scopes or use a person/member author. Development is a test tier; Standard tier and its use-case screencast review remain required before production rollout. The app-name issue observed on 2026-08-10 remains a provider-portal blocker until a submitted app is externally verified; it is not evidence of configured access.
