...
export default async function ProfilePage({ searchParams }: { searchParams: Promise<{ email?: string }> }) {
  ...
  const user = email ? await db.user.findUnique({ where: { email }, include: { subscription: true, emailLogs: true, dialogues: true } }) : null;
      ...
      <div className="mx-auto grid max-w-5xl gap-6 lg:grid-cols-3">
        <section className="card p-6 lg:col-span-1">
          ...
          <form className="mt-4 space-y-3">
            ...
            <input className="w-full rounded bg-slate-950 p-2" placeholder="Avatar URL" defaultValue={user?.avatarUrl ?? ""} />
            <button type="button" className="btn-primary w-full">Kaydet</button>
          </form>
        </section>
        <section className="card p-6 lg:col-span-2">
          <h2 className="text-xl font-semibold">Hesap Özeti</h2>
          {!user ? <p className="mt-3 text-slate-300">`?email=` query param ile profil verisi görüntülenir (MVP).</p> : (
            <div className="mt-4 grid gap-3 sm:grid-cols-2">
              ...
              <div className="rounded border border-white/10 p-3">Son giriş: {user.lastLoginAt?.toISOString() ?? "-"}</div>
              <div className="rounded border border-white/10 p-3">Aldığı email: {user.emailLogs.length}</div>
              <div className="rounded border border-white/10 p-3">AI konuşma: {user.dialogues.length}</div>
            </div>
          )}
        </section>
      </div>
