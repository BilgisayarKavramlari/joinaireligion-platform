...
export const AGENT_DEFINITIONS: AgentDefinition[] = [
  ...
  },
  {
    ...
    description: "Builds multilingual SEO content and sends passing candidates to a separate independent publication gate.",
    lifecycle: "IMPLEMENTED",
    mode: "DRAFT_ONLY",
    ...
    policy: {
      autonomyLevel: 1,
      allowedActions: ["research aggregate trends", "draft multilingual content", "organize content queue", "flag risks"],
  ...
  },
  {
    ...
    description: "Aggregates anonymous engagement and feeds category performance back into future topic selection.",
    lifecycle: "IMPLEMENTED",
    mode: "LIVE",
    ...
    policy: {
      autonomyLevel: 2,
      allowedActions: ["aggregate anonymous content metrics", "update performance scores", "reversibly unpublish content with strong negative signals", "create performance reports"],
      forbiddenActions: ["store raw IP addresses", "profile individual users", "delete content", "change publication safety thresholds at runtime"],
      escalationConditions: ["unexpected metric volume", "database growth anomaly", "repeated automatic unpublishing"],
      defaultSafeBoundaries: ["aggregate-only learning", "minimum sample thresholds", "reversible unpublish", "no deletion"],
      decisionLog: DECISION_LOG_CONTRACT,
    },
  },
  {
  ...
  },
  {
    ...
    description: "Prepares one multilingual daily social package from independently reviewed, published site content.",
    lifecycle: "IMPLEMENTED",
    mode: "DRAFT_ONLY",
    ...
    policy: {
      autonomyLevel: 1,
      allowedActions: ["read published site content", "draft social posts", "create idempotent publication packages", "flag content risk"],
  ...
  },
  {
    ...
    description: "Publishes approved social packages to configured provider accounts with idempotency and per-provider delivery logs.",
    lifecycle: "IMPLEMENTED",
    mode: "LIVE",
    ...
    policy: {
      ...
      defaultSafeBoundaries: ["explicit production switch required", "configured providers only", "idempotent provider delivery", "no engagement actions"],
      decisionLog: DECISION_LOG_CONTRACT,
    },
  },
  {
